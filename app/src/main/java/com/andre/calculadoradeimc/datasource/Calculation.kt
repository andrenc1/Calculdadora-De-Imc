...
object Calculation {
    ...
    fun calculateIMC(peso: String, altura: String, response: (String, String, Boolean) -> Unit) {
        ...
        val classificacao = when {
            ...
            imc < 40 -> "Obesidade Severa (Grau II)"
            else -> "Obesidade Mórbida (Grau III)"
        }

        val resultadoFinal = "IMC: $imcFormatado\n$classificacao"
        response(resultadoFinal, classificacao, false)
    }
    ...
    fun calculateTMB(
        ...
        val (interpretacao, zonaDeRisco) = when {
            ...
            imc < 25 -> "Peso Normal" to "✅ Parabéns! Você está em uma faixa saudável."
            imc < 30 -> "Sobrepeso" to "!Atenção!: Excesso de peso moderado."
            imc < 35 -> "Obesidade Grau I" to "🚨 Cuidado: Obesidade leve, risco aumentado."
            imc < 40 -> "Obesidade Severa (Grau II)" to "🚨 Alerta: Obesidade severa, procure um médico."
